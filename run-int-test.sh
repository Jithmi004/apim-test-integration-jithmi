...
mkdir -p "$INT_TEST_MODULE_DIR/tests-integration/tests-benchmark/logs/benchmark-tests"
cd $INT_TEST_MODULE_DIR  && mvn clean install -fae -B -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=warn -Ptestgrid -pl tests-integration/tests-restart,tests-integration/tests-benchmark -am -Dhttp.keepAlive=false
